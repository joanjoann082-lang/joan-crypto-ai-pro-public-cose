...
class StrategyEngine:
    def candidates(self, ctx: Dict[str, Any]) -> List[Candidate]:
        ...
        raw=50 + fnum(tf4h.get('score'))*0.25 + fnum(tf1h.get('score'))*0.35 - max(0,fnum(tf15.get('rsi'))-68)*0.45
        reason=[]
        ...
        raw2=48 - fnum(tf15.get('score'))*0.15 - fnum(tf1h.get('score'))*0.10 + max(0,35-fnum(tf15.get('rsi'),50))*0.8 + max(0, fnum(der.get('long_liq_usd'))/1_000_000)*2
        ...
        raw3=50 - fnum(tf4h.get('score'))*0.25 - fnum(tf1h.get('score'))*0.35 - max(0,32-fnum(tf15.get('rsi')))*0.4
        reason3=[]
