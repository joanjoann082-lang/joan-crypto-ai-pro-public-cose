...
VERSION = "PAPER_MICRO_CANARY_BRIDGE_V10_NET_RISK"
...
class PaperMicroCanaryBridgeV10:
    """
    Isolated V10 paper micro-canary bridge.

    Writes only paper_micro_canary_positions_v10 and audit table.
    ...
    def ensure_schema(self) -> None:
        self.db.execute("""
            CREATE TABLE IF NOT EXISTS paper_micro_canary_positions_v10 (
        ...
        """)
        self.db.execute("""
            CREATE TABLE IF NOT EXISTS paper_micro_canary_audit_v10 (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ts TEXT NOT NULL,
    ...
    def audit(self, event: str, level: str, message: str, payload: Dict[str, Any]) -> None:
        self.db.execute("""
            INSERT INTO paper_micro_canary_audit_v10 (ts, version, event, level, message, payload)
            VALUES (?, ?, ?, ?, ?, ?);
    ...
    def latest_control(self) -> Dict[str, Any]:
        return self.q1("SELECT * FROM latest_institutional_control_plane_v10;")

    def open_canary(self) -> Dict[str, Any]:
        return self.q1("SELECT * FROM paper_micro_canary_positions_v10 WHERE status='OPEN' ORDER BY id DESC LIMIT 1;")
    ...
    def manage_open(self, prices: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if res["net_r"] <= -1.0:
            close = True
            reason = "V10_CANARY_STOP_NET_-1R"
        elif res["net_r"] >= 1.5:
            close = True
            reason = "V10_CANARY_TAKE_PROFIT_NET_1_5R"
        elif age_min >= max(10, horizon):
            close = True
            reason = "V10_CANARY_HORIZON_CLOSE"

        if close:
            self.db.execute("""
                UPDATE paper_micro_canary_positions_v10
        ...
        self.db.execute("UPDATE paper_micro_canary_positions_v10 SET mfe_r=?, mae_r=?, pnl_usd=?, pnl_r=?, net_pnl_usd=?, net_pnl_r=? WHERE id=?;",
    ...
    def maybe_open(self, prices: Dict[str, Any], allow_open: bool = True) -> Dict[str, Any]:
        ...
        if not c:
            return {"opened": False, "reason": "NO_CONTROL_V10"}
        if not allow_open:
        ...
        self.db.execute("""
            INSERT INTO paper_micro_canary_positions_v10 (
        ...
        """, (
            ...
            "CONTROL_V10_PAPER_MICRO_CANARY_READY", js(payload),
        ))
        self.audit("CANARY_OPENED", "INFO", "V10 paper micro canary opened", {
...
def main() -> None:
    ...
    engine = PaperMicroCanaryBridgeV10()
