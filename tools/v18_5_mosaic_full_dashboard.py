...
VERSION = "V18.5 MOSAIC FULL SCREEN + LAST TRADES"
REFRESH_SEC = 60
BLOCKS = "▁▂▃▄▅▆▇█"
...
def result_bar(r, width):
    v = clamp(fnum(r, 0.0) or 0.0, -2.0, 2.0)
    mid = width // 2
    arr = ["░"] * width
    arr[mid] = "│"
    if v >= 0:
        n = int((v / 2.0) * (width - mid - 1))
        for i in range(mid + 1, min(width, mid + 1 + n)):
            arr[i] = "█"
    else:
        n = int((abs(v) / 2.0) * mid)
...
def load_last_trades(con):
    ...
    if exists(con, "trades"):
        c = cols(con, "trades")
        ts_col = "closed_at" if "closed_at" in c else "ts" if "ts" in c else "opened_at" if "opened_at" in c else "id"
        pnl_r_col = "net_pnl_r" if "net_pnl_r" in c else "pnl_r" if "pnl_r" in c else None
        pnl_usd_col = "net_pnl_usd" if "net_pnl_usd" in c else "pnl_usd" if "pnl_usd" in c else None
    ...
    def sort_key(r):
        return str(r.get("closed_at") or r.get("opened_at") or "")
...
def load():
    ...
    try:
        ...
        d["last_trades"] = load_last_trades(con)
...
def trade_state_label(r):
    status = str(r.get("status") or "").upper()
    net_r = fnum(r.get("net_r"), None)

    if status in {"OPEN", "ACTIVE", "RUNNING"}:
        return "OPEN"
    if net_r is None:
        return "?"
    if net_r > 0:
...
def draw_last_trades(stdscr, y, x, h, w, d, C):
    box(stdscr, y, x, h, w, "LAST 10 TRADES", C["cyan"])

    trades = d["last_trades"]
    if not trades:
        add(stdscr, y+2, x+2, "No trades yet", C["yellow"])
        return

    row_y = y + 1
    max_rows = max(1, h - 2)

    for r in trades[:max_rows]:
        if row_y >= y + h - 1:
            break

        label = trade_state_label(r)
        ...
        if w >= 85:
            add(stdscr, row_y, x+2, f"{icon} {label:<5}", col | curses.A_BOLD)
            add(stdscr, row_y, x+10, short(f"{symbol} {side} {setup}", 34), curses.A_BOLD)
            add(stdscr, row_y, x+46, fmt_r(net_r), col | curses.A_BOLD)
            add(stdscr, row_y, x+55, result_bar(net_r, max(10, w-78)), col)
            add(stdscr, row_y, x+w-19, short(fmt_usd(net_usd), 8), col)
            add(stdscr, row_y, x+w-10, short(reason, 8))
        else:
            add(stdscr, row_y, x+2, f"{icon} {label:<5} {symbol} {side} {fmt_r(net_r)}", col | curses.A_BOLD)
            add(stdscr, row_y, x+31, result_bar(net_r, max(8, w-34)), col)
...
def draw(stdscr):
    ...
    while True:
        ...
        remaining = h - y - bottom_h - footer_h - 2
        remaining = max(12, remaining)

        if remaining >= 24:
            quant_h = max(8, int(remaining * 0.48))
            trades_h = remaining - quant_h - 1
        else:
            quant_h = max(6, remaining // 2)
            trades_h = max(6, remaining - quant_h - 1)

        draw_quant_card(stdscr, y, margin, quant_h, usable_w, d, C)
        y += quant_h + 1
        ...
        add(stdscr, h-1, 1, short("q sortir | r refresh | full-screen mosaic | last 10 trades | no execution", w-3), curses.A_REVERSE)
